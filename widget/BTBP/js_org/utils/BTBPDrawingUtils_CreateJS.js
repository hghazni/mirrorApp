...
(function () {
	...
	var drawing = btbpExample.drawing;
	...
	drawing.drawFoundation = function (imageData, bounds, vertices, clear, isComplexion) {
		clear = defaultValue(clear, false);

		var g = drawing.draw;
		clear && g.clear();

		btbpExample.features.applyFoundation(imageData, drawing.faceTextures, bounds, vertices, isComplexion);
	};

	drawing.drawComplexion = function (imageData, bounds, vertices, clear) {
		clear = defaultValue(clear, false);

		var g = drawing.draw;
		clear && g.clear();

		btbpExample.features.applyComplexion(imageData, drawing.faceTextures, bounds, vertices);
	};

	drawing.drawLipStick = function (imageData, bounds, vertices, clear) {
		clear = defaultValue(clear, false);

		var g = drawing.draw;
		clear && g.clear();

		if (settings.Foundation || settings.Complexion)
			btbpExample.features.applyLipstick(drawing.faceTextures, drawing.faceTextures, bounds, vertices);
		else
			btbpExample.features.applyLipstick(imageData, drawing.faceTextures, bounds, vertices);
	};
	...
	drawing.fillTriangles = function (vertices, triangles, clear, fillColor, fillAlpha) {
		...
		//var lineColor = getColor(fillColor, fillAlpha-0.4);		
		fillColor = getColor(fillColor, fillAlpha);
